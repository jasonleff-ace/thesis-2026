...
```{r}
best_k <- which.max(sil_width)
clusters_best <- cutree(hc, k = 5)

plot(x, rep(0, length(x)), col = clusters_best, pch = 19,
     xlab = "NBA_3P%", ylab = "",
     yaxt = "n", main = paste("Hierarchical Clustering on NBA_FG3_PCT (k =", 5, ")"))
```
```{r}
cluster_medians <- tapply(x, clusters_best, median)
ordered_clusters <- names(sort(cluster_medians, decreasing = FALSE))
...
```
```{r}
pc_importance <- abs(svm_linear$finalModel$W)
pc_importance
rotation <- svm_linear$preProcess$rotation   # PCA loadings

# PC importance from SVM
pc_imp <- abs(svm_linear$finalModel$W)

# Map back to original variables
var_importance <- abs(rotation %*% pc_imp)

# Convert to named vector
var_importance <- as.vector(var_importance)
names(var_importance) <- rownames(rotation)
...
y <- factor(y, levels = 1:5, labels = c("Bad", "BelowAvg", "Average", "AboveAvg", "Great"))
...
train_data <- df_scaled[train_index, ]
test_data  <- df_scaled[-train_index, ]
class_levels <- c("Bad", "BelowAvg", "Average", "AboveAvg", "Great")
```
...
ggplot(cm_table, aes(x = Reference, y = Prediction, fill = Freq)) +
  ...
  theme_minimal() +
  labs(title = "Confusion Matrix Heatmap of 3P% Classes (LDA) 56% Accuracy",
       x = "Actual Class", y = "Predicted Class")
```
...
y <- factor(y, levels = 1:5, labels = c("ZeroAvgPCT","BelowAvgPCT","AveragePCT", "AboveAvgPCT", "HighPCT"))
...
train_data <- df_scaled[train_index, ]
test_data  <- df_scaled[-train_index, ]
class_levels <- c("ZeroAvgPCT","BelowAvgPCT","AveragePCT", "AboveAvgPCT", "HighPCT")
```
...
```
```{r}
predictors <- df_scaled[, c("PRE_G", "PRE_3PA", "PRE_3P_PCT", "PRE_FTA", "PRE_FT_PCT")]
cor_matrix <- cor(predictors)
print(round(cor_matrix, 3))
```
No issues
