...
```{r}
ft_model <- lm(response ~ PRE_FT_PCT, data = train_data)
ft_pred <- predict(ft_model, newdata = test_data)
ft_metrics <- calculate_metrics(ft_pred, y_test)
...
ThreeP_model <- lm(response ~ PRE_3P_PCT, data = train_data)
ThreeP_pred <- predict(ThreeP_model, newdata = test_data)
ThreeP_metrics <- calculate_metrics(ThreeP_pred, y_test)

model_results <- rbind(model_results, data.frame(
  Model = "SLR on 3P%",
  RMSE = ThreeP_metrics[1],
  MAE = ThreeP_metrics[2],
  R2 = ThreeP_metrics[3]
))

both_model <- lm(response ~ PRE_FT_PCT + PRE_3P_PCT, data = train_data)
both_pred <- predict(both_model, newdata = test_data)
both_metrics <- calculate_metrics(both_pred, y_test)

model_results <- rbind(model_results, data.frame(
  Model = "MLR on FT% and 3P%",
  RMSE = both_metrics[1],
  MAE = both_metrics[2],
  R2 = both_metrics[3]
))
...
# Get t-values (excluding intercept)
tvals <- s$coefficients[-1, "t value"]

# Sort descending
importance <- sort(tvals, decreasing = TRUE)
...
importance_df$Type <- ifelse(grepl("_sq$", importance_df$Feature),
                             "Squared Term",
                             "Linear Term")


top10 <- importance_df[1:10,]
ggplot(top10, aes(x = reorder(Feature, Importance),
  ...
  coord_flip() +
  labs(
    title = "Variable Importance of Predicting 3 Point Made Per Minute (Top 10 of Quadratic OLS)",
    x = "Feature",
    y = "Absolute t-value",
    fill = "Term Type"
  ) + 
...
# Get coefficients and t-values (excluding intercept)
coef_vals <- s$coefficients[-1, "Estimate"]
tvals <- s$coefficients[-1, "t value"]

# Sort descending by absolute t-value
importance <- sort(abs(tvals), decreasing = TRUE)
importance_df <- data.frame(
  Feature = names(importance),
  Importance = importance,
  Coefficient = coef_vals[names(importance)]
)

# Determine color based on sign and term type
importance_df$ColorGroup <- with(importance_df,
  ifelse(Coefficient > 0,
    ifelse(grepl("_sq$", Feature), "lightblue", "blue"),
    ifelse(grepl("_sq$", Feature), "lightcoral", "red")
  )
)

# Create fill mapping
fill_colors <- c(
  "blue" = "#1f77b4",      # Positive linear
  "lightblue" = "#6baed6", # Positive squared
  "red" = "#d62728",       # Negative linear
  "lightcoral" = "#f0868e" # Negative squared
)

# Get top 10
top10 <- importance_df[1:10,]

p<- ggplot(top10, aes(x = reorder(Feature, Importance),
                  y = Importance,
                  fill = ColorGroup)) +
  geom_col() +
  scale_fill_manual(
    values = fill_colors,
    labels = c(
      "blue" = "Positive Coefficient (Linear)",
      "lightblue" = "Positive Coefficient (Squared)",
      "red" = "Negative Coefficient (Linear)",
      "lightcoral" = "Negative Coefficient (Squared)"
    ),
...
ggsave("variable_importance.png", plot = p, width = 10, height = 6, dpi = 300)
```
...
p1_full <- plot_ly() %>%
  ...
  add_trace(x = c(min(full_plot_data$Actual), max(full_plot_data$Actual)), 
            ...
            name = 'Perfect Fit',
            showlegend = TRUE) %>%
  layout(title = 'Actual vs Predicted Three Point Made Per Minute (Full Dataset OLS)',
         xaxis = list(title = 'Actual 3PMPM'),
         yaxis = list(title = 'Predicted 3PMPM'),
         legend = list(x = 0.02, y = 0.98, font = list(size = 16),
...
p2_full <- plot_ly() %>%
  ...
  add_trace(x = c(min(full_plot_data$Actual), max(full_plot_data$Actual)), 
            ...
            name = 'Perfect Fit') %>%
  layout(title = 'NBA Three Point Made Per Minute Colored by Error Magnitude (Full Dataset)',
         xaxis = list(title = 'Actual 3PMPM'),
         yaxis = list(title = 'Predicted 3PMPM'))
