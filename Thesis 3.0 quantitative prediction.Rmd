...
# OLS polynmials with CV
```{r}
set.seed(67)
cv_folds <- createFolds(train_data$response, k = 10)
cv_rmse_poly_ols <- numeric(10)
cv_mae_poly_ols <- numeric(10)
cv_r2_poly_ols <- numeric(10)
...
for(i in 1:10) {
  ...
  cv_rmse_poly_ols[i] <- sqrt(mean((fold_pred - valid_fold$response)^2))
  cv_mae_poly_ols[i] <- mean(abs(fold_pred - valid_fold$response))
  cv_r2_poly_ols[i] <- 1 - sum((valid_fold$response - fold_pred)^2) / 
    sum((valid_fold$response - mean(valid_fold$response))^2)
}
```
...
model_results_cv <- rbind(model_results_cv, 
  ...
  data.frame(
    Model = "Linear Regression with polynomial terms (10-fold CV)",
    RMSE = mean(cv_rmse_poly_ols),
    MAE = mean(cv_mae_poly_ols),
    R2 = mean(cv_r2_poly_ols)
  ),
  data.frame(
